...
gcap_status_t CaptureManager::exportPreviewSceneRgb10(const char *basePathUtf8, int rawFlags, bool exportTiff, bool exportStats, bool exportGigabyteRaw)
{
    if (!provider_ || !basePathUtf8 || !*basePathUtf8)
        return GCAP_EINVAL;
    return provider_->exportPreviewSceneRgb10(basePathUtf8, rawFlags, exportTiff, exportStats, exportGigabyteRaw) ? GCAP_OK : GCAP_ENOTSUP;
}
